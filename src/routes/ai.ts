...
async function callOpenAI(prompt: string, maxTokens: number = 500) {
  ...
  try {
    ...
    try {
      ...
      if (jsonMatch) {
        ...
        tags = tagsResponse.split(',').map((tag:any)=> tag.trim().replace(/["'\[\]]/g, ''))
      }
    } catch {
...
function generateFallbackTags(content: string, title?: string): string[] {

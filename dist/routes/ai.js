...
const aiSummarySchema = zod_1.z.object({
    content: zod_1.z.string().min(1, 'Content is required'),
});
const aiImproveSchema = zod_1.z.object({
    content: zod_1.z.string().min(1, 'Content is required'),
});
const aiTagsSchema = zod_1.z.object({
    content: zod_1.z.string().min(1, 'Content is required'),
    title: zod_1.z.string().optional(),
});
const aiTitleSchema = zod_1.z.object({
    content: zod_1.z.string().min(1, 'Content is required'),
});
...
app.post('/summarize', auth_1.authMiddleware, (0, validation_1.validate)(aiSummarySchema), async (c) => {
...
app.post('/improve', auth_1.authMiddleware, (0, validation_1.validate)(aiImproveSchema), async (c) => {
    const { content } = c.get('validatedBody');
    try {
...
app.post('/generate-tags', auth_1.authMiddleware, (0, validation_1.validate)(aiTagsSchema), async (c) => {
...
app.post('/generate-title', auth_1.authMiddleware, (0, validation_1.validate)(aiTitleSchema), async (c) => {
    const { content } = c.get('validatedBody');
    try {
...
app.get('/health', auth_1.authMiddleware, async (c) => {
    try {
